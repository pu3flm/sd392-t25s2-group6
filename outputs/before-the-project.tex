...
The distinction also clarifies why provenance is insufficient. PROV-DM defines the W3C provenance data model for entities, activities, agents, derivation, revision, attribution, and delegation; PROV-O supplies its OWL2 encoding \cite{provdm2013,provo2013}. A PROV description can show that an agent generated an artifact from another artifact. It does not, by itself, establish that the derivation preserved stakeholder purpose, that the agent was authorized to perform it, or that the result satisfies the intended criterion. Provenance answers lineage questions; intention answers relevance questions; legitimacy requires a trace from the author of the governing purpose, through a bounded mandate, to the agent permitted to act.\footnote{The paper therefore does not use \emph{authority} as a loose synonym for authorship, institutional standing, authorization, permission, executable capability, or the actor that holds a role. The words \emph{author} and \emph{authority} share a history through Latin \emph{auctor} and \emph{auctoritas}; the architectural point is not that etymology fixes present meaning, but that origination, delegation, and execution must remain distinguishable. The author of an intention, the mandate or grant derived from it, and the executor acting under that grant are separate nodes or relations; responsibility and accountability require their own treatment.}
